...
import { startCloudRun } from './jobs';
...
/**
 * Zet campagnetekst om in een campagne: AI-parsing waar dat kan, de
 * heuristische parser als er geen Claude-backend is. Gedeeld door de
 * plak-import op het dashboard en de bookmarklet vanaf cliparmy.nl.
 */
export async function importeerCampagneTekst(tekst: string) {
  let parsed: z.infer<typeof campagneSchema>;
  try {
    parsed = await structuredCall({
      system: IMPORT_SYSTEM,
      user: tekst.trim().slice(0, 20000),
    ...
    const geenClaude = /Ontbrekende env var|CLI niet gevonden|niet \(meer\) ingelogd|ENOENT/i.test(
      e instanceof Error ? e.message : String(e),
    );
    if (!geenClaude) throw e;
    parsed = heuristischeParse(tekst.trim());
  }
  ...
  return { campaign: data, onduidelijk: parsed.onduidelijk, bronKanalen: parsed.bron_kanalen, kanaalGestart };
}
